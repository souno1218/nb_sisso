    ...
    if max_op != 7:
        ...
        for n_op1 in range(max_op - 1, -1, -1):
            ...
            if os.path.isfile(f"cache_folder/saved_7_make_unique_equations_thread_{n_op1}_{num_threads}.npz"):
                TF_list = np.load(f"cache_folder/saved_7_make_unique_equations_thread_{n_op1}_{num_threads}.npz")[
                    "TF_list"
                ]
                how_loop = np.sum(TF_list)
...
def make_unique_equations_info_7(n_op1, TF_list, random_x, progress_proxy):
    ...
    return_need_calc_change_x = np.zeros((sum_TF, tot_mask_x.shape[0]), dtype="bool")
    with objmode():
        print(f"         using memory : {psutil.Process().memory_info().rss / 1024**2} M bytes")
    for thread_id in prange(num_threads):
        equation = np.full((2 * max_op + 1), Nan_number, dtype="int8")
        cache_for_mask_x = np.full((tot_mask_x.shape[0], 2, random_x.shape[1]), Nan_number, dtype="float64")
        last_index = np.sum(TF_list[:thread_id])
