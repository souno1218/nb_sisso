...
@njit(error_model="numpy")
def find_similar_num(arr,mat):#x.shape=(N),y=ans
    rtol=1e-06
...
@njit(error_model="numpy")
def find_similar_num_index(arr,mat):#x.shape=(N),y=ans
    rtol=1e-06
    for i in range(mat.shape[0]):
        if is_close_arr(arr,mat[i]):
            return i
...
def find_min_x_max(equation,random_x,random_for_find_min_x_max):
    ...
    change_num=np.arange(max_x+1)[counter>1][np.argsort(counter[counter>1])[::-1]]
    ...
    for i in change_num:
        ...
        if (np.abs(similar_num[0]-changed_similar_num[0])<=rtol*np.abs(changed_similar_num[0])):
            if np.all(np.abs(similar_num-changed_similar_num)<=rtol*np.abs(changed_similar_num)):
                return True
...
def make_before_similar_num_list(num_threads,max_op,random_x,before_equations,progress_proxy):
    ...
    save_similar_num_list=np.full((num_threads,2,loop_per_threads,random_x.shape[1]),Nan_number,dtype="float64")
    last_index=np.zeros((num_threads),dtype="int64")
    for thread_id in prange(num_threads):
        thread_similar_num_list=np.full((2,loop_per_threads,random_x.shape[1]),Nan_number,dtype="float64")
    ...
    return_similar_num_list=np.full((2,np.sum(last_index),random_x.shape[1]),Nan_number,dtype="float64")
    tot_last_index=0
    for thread_id in range(num_threads):
        return_similar_num_list[0,tot_last_index:tot_last_index+last_index[thread_id]]=save_similar_num_list[thread_id,0,:last_index[thread_id]]
        return_similar_num_list[1,tot_last_index:tot_last_index+last_index[thread_id]]=save_similar_num_list[thread_id,1,:last_index[thread_id]]
...
@njit(parallel=True,error_model="numpy")
def make_unique_equations_thread(max_op,n_op1,num_threads,random_x,before_similar_num_list,saved_similar_num_list,progress_proxy):
    Nan_number=-100
    n_op2=max_op-1-n_op1
    ...
    save_similar_num_list=np.full((num_threads,2,loop_per_threads,random_x.shape[1]),Nan_number,dtype="float64")
    head_save_similar_num_list=np.full((num_threads,loop_per_threads),Nan_number,dtype="float64")
    ...
    if n_op1>=n_op2:
        ...
        ops=[-4]    
    with objmode():
        print("         using memory : ",psutil.Process().memory_info().rss / 1024 ** 2, "M bytes",flush=True)
    for thread_id in prange(num_threads):
        equation=np.full((2*max_op+1),Nan_number,dtype="int8")
        cache_for_mask_x=np.full((tot_mask_x.shape[0],2,random_x.shape[1]),Nan_number,dtype="float64")
        counter=0
        loop=base_eq_arr1.shape[0]*base_eq_arr2.shape[0]
        for i in range(thread_id,loop,num_threads):
            id1=i%base_eq_arr1.shape[0]
            ...
            for merge_op in ops:
                ...
                for number in range(dict_max_loop[base_back_x_max,flont_x_max]):
                    ...
                    equation[len_base_eq1:len_base_eq1+len_base_eq2]=make_eq(base_back_equation,dict_change_x_pattern[base_back_x_max][number])
                            ...
                            if k==0:
                                if similar_num[0,0]==Nan_number:
                                    if np.sum(equation>0)!=0:
                                        ...
                                        is_calc=False
                            elif find_similar_num(similar_num[1],cache_for_mask_x[:n,1]):
                                continue
                            if is_save:
                    ...
                    if is_save:
                        ...
                        if max_op+1!=eq_x_max:
                            same_head_index=is_close_arr_index(cache_for_mask_x[normalized_min_index,0,0],head_before_similar_num_list)
                            for t in same_head_index:
                                if is_close_arr(cache_for_mask_x[normalized_min_index,0],before_similar_num_list[0,t]):
                                    is_calc=False
                                    if is_close_arr(cache_for_mask_x[min_index,1],before_similar_num_list[1,t]):
                                        is_save=False
                                        break
                    if is_save:
                        same_head_index=is_close_arr_index(cache_for_mask_x[normalized_min_index,0,0],head_saved_similar_num_list)
                        for t in same_head_index:
                            if is_close_arr(cache_for_mask_x[normalized_min_index,0],saved_similar_num_list[0,t]):
                                is_calc=False
                                if is_close_arr(cache_for_mask_x[min_index,1],saved_similar_num_list[1,t]):
                                    is_save=False
                                    break
                    if is_save:
                        same_head_index=is_close_arr_index(cache_for_mask_x[normalized_min_index,0,0],head_save_similar_num_list[thread_id,:counter])
                        for t in same_head_index:
                            if is_close_arr(cache_for_mask_x[normalized_min_index,0],save_similar_num_list[thread_id,0,t]):
                                is_calc=False
                                if is_close_arr(cache_for_mask_x[min_index,1],save_similar_num_list[thread_id,1,t]):
                                    is_save=False
                                    break
                    if is_save:
...
def dim_reduction(TF_list,similar_num_list,need_calc_list,progress_proxy):
    ...
    return_similar_num_list=np.full((2,np.sum(TF_list),similar_num_list.shape[3]),Nan_number,dtype="float64")
    return_similar_num_list[0,:np.sum(TF_list[sort_index[0]])]=similar_num_list[sort_index[0],0,TF_list[sort_index[0]]]
    return_similar_num_list[1,:np.sum(TF_list[sort_index[0]])]=similar_num_list[sort_index[0],1,TF_list[sort_index[0]]]
    head_return_similar_num_list=np.full((np.sum(TF_list)),Nan_number,dtype="float64")
    head_similar_num_list=similar_num_list[:,0,:,0]
    head_return_similar_num_list[:np.sum(TF_list[sort_index[0]])]=head_similar_num_list[sort_index[0],TF_list[sort_index[0]]]
    ...
    for target_index in sort_index[1:]:
        ...
        for thread_id in prange(num_threads):
            ...
            for i in true_index_thread:
                for t in is_close_arr_index(similar_num_list[target_index,0,i,0],head_return_similar_num_list[:last_index]):
                    if is_close_arr(similar_num_list[target_index,0,i],return_similar_num_list[0,t]):
                        need_calc_list[target_index,i]=False
                        if is_close_arr(similar_num_list[target_index,1,i],return_similar_num_list[1,t]):
                            TF_list[target_index,i]=False
                            break
                progress_proxy.update(1)
        return_similar_num_list[0,last_index:last_index+np.sum(TF_list[target_index])]=similar_num_list[target_index,0,TF_list[target_index]]
        return_similar_num_list[1,last_index:last_index+np.sum(TF_list[target_index])]=similar_num_list[target_index,1,TF_list[target_index]]
        head_return_similar_num_list[last_index:last_index+np.sum(TF_list[target_index])]=head_similar_num_list[target_index,TF_list[target_index]]
...
def make_unique_equations_info(max_op,n_op1,TF_list,need_calc_list,random_x,progress_proxy):
    ...
    return_similar_num_list=np.full((2,sum_TF,random_x.shape[1]),Nan_number,dtype="float64")
            ...
            for merge_op in ops:
                ...
                for number in range(dict_max_loop[base_back_x_max,flont_x_max]):
                    ...
                    if TF_list_thread[counter]:
                        ...
                        for k in range(mask_x.shape[0]):
                            similar_num=nb_calc_RPN(random_x[mask_x[k]],equation)
                            if find_similar_num(similar_num[1],cache_for_mask_x[:n,1]):
                        ...
                        changed_back_eq_id=make_change_x_id(dict_change_x_pattern[base_back_x_max][number],max_op+1)
                        return_base_eq_id_list[last_index,4]=changed_back_eq_id
                        return_need_calc_change_x[last_index,:mask_x.shape[0]]=thread_need_calc_change_x[:mask_x.shape[0]]
                        last_index+=1
                        progress_proxy.update(1)
                    counter+=1
    return return_equation_list,return_similar_num_list,return_need_calc_list,return_base_eq_id_list,return_need_calc_change_x

@njit(parallel=True,error_model="numpy")
def make_unique_equations_thread_7(n_op1,num_threads,random_x,before_similar_num_list,saved_similar_num_list,progress_proxy):
    Nan_number=-100
    max_op=7
    n_op2=max_op-1-n_op1
    ...
    save_similar_num_list=np.full((num_threads,loop_per_threads,random_x.shape[1]),Nan_number,dtype="float64")
    head_save_similar_num_list=np.full((num_threads,loop_per_threads),Nan_number,dtype="float64")
    TF_list=np.zeros((num_threads,loop_per_threads),dtype="bool")
    if n_op1>=n_op2:
        ops=[-1,-2,-3,-4]
                ...
                for number in range(dict_max_loop[base_back_x_max,flont_x_max]):
                    ...
                    for k in range(mask_x.shape[0]):
                        ...
                        if is_save:
                            ...
                            if k==0:
                                if similar_num[0,0]==Nan_number:
                                    is_save=False
                            elif find_similar_num(similar_num[0],cache_for_mask_x[:n,0]):
                                continue
                            if is_save:
                                cache_for_mask_x[n]=similar_num
                                n+=1
                    ...
                    if is_save:
                        ...
                        if max_op+1!=eq_x_max:
                            same_head_index=is_close_arr_index(cache_for_mask_x[normalized_min_index,0,0],head_before_similar_num_list)
                            for t in same_head_index:
                                if is_close_arr(cache_for_mask_x[normalized_min_index,0],before_similar_num_list[t]):
                                    is_save=False
                                    break
                    if is_save:
                        same_head_index=is_close_arr_index(cache_for_mask_x[normalized_min_index,0,0],head_saved_similar_num_list)
                        for t in same_head_index:
                            if is_close_arr(cache_for_mask_x[normalized_min_index,0],saved_similar_num_list[t]):
                                is_save=False
                                break
                    if is_save:
                        same_head_index=is_close_arr_index(cache_for_mask_x[normalized_min_index,0,0],head_save_similar_num_list[thread_id,:counter])
                        for t in same_head_index:
                            if is_close_arr(cache_for_mask_x[normalized_min_index,0],save_similar_num_list[thread_id,t]):
                                is_save=False
                                break
                    if is_save:
                        TF_list[thread_id,counter]=True
...
def dim_reduction_7(TF_list,similar_num_list,progress_proxy):
    ...
    threads_last_index=np.array([np.sum(TF_list[i]) for i in range(num_threads)])
    sort_index=np.argsort(threads_last_index)[::-1]
    return_similar_num_list=np.full((np.sum(TF_list),similar_num_list.shape[2]),Nan_number,dtype="float64")
    ...
    for target_index in sort_index[1:]:
        ...
        for thread_id in prange(num_threads):
            true_index_thread=true_index[thread_id::num_threads].copy()
            for i in true_index_thread:
                for t in is_close_arr_index(similar_num_list[target_index,i,0],head_return_similar_num_list[:last_index]):
                    if is_close_arr(similar_num_list[target_index,i],return_similar_num_list[t]):
                        TF_list[target_index,i]=False
                        break
                progress_proxy.update(1)
        return_similar_num_list[last_index:last_index+np.sum(TF_list[target_index])]=similar_num_list[target_index,TF_list[target_index]]
        head_return_similar_num_list[last_index:last_index+np.sum(TF_list[target_index])]=head_similar_num_list[target_index,TF_list[target_index]]
        last_index+=np.sum(TF_list[target_index])
    return TF_list
...
def make_unique_equations_info_7(n_op1,TF_list,random_x,progress_proxy):
    ...
    sum_TF=np.sum(TF_list)
    num_threads=TF_list.shape[0]
    return_similar_num_list=np.full((sum_TF,random_x.shape[1]),Nan_number,dtype="float64")
    return_equation_list=np.full((sum_TF,2*max_op+1),Nan_number,dtype="int8")
    return_base_eq_id_list=np.full((sum_TF,5),Nan_number,dtype="int64")
    return_need_calc_change_x=np.zeros((sum_TF,tot_mask_x.shape[0]),dtype="bool")
    with objmode():
            ...
            for merge_op in ops:
                ...
                for number in range(dict_max_loop[base_back_x_max,flont_x_max]):
                    if TF_list_thread[counter]:
                        ...
                        progress_proxy.update(1)
                    counter+=1
    return return_equation_list,return_similar_num_list,return_base_eq_id_list,return_need_calc_change_x
...
def make_similar_num_by_saved_7(n_op1,TF_list,random_x,progress_proxy):
    ...
    if n_op1>=n_op2:
        ...
        ops=[-4]
    num_threads=TF_list.shape[0]
    return_similar_num_list=np.full((num_threads,TF_list.shape[1],random_x.shape[1]),Nan_number,dtype="float64")
    with objmode():
        print("         using memory : ",psutil.Process().memory_info().rss / 1024 ** 2, "M bytes",flush=True)
    for thread_id in prange(num_threads):
        equation=np.full((2*max_op+1),Nan_number,dtype="int8")
...
def make_final_cache(max_ops,num_threads,equation_list,need_calc_list,base_eq_id_list,need_calc_change_x):
    ...
    print(f"   max_id_need_calc, {list(max_id_need_calc)} -> {list(base)}",flush=True)
    ...
    for n_op1 in range(max_ops-1,-1,-1):
        ...
        if n_op1>=n_op2:
            cache=np.full((base_dict[n_op1].shape[0],base_dict[n_op2].shape[0],unique_num.shape[0],4),Nan_number,dtype="int64")
        else:
            cache=np.full((base_dict[n_op1].shape[0],base_dict[n_op2].shape[0],unique_num.shape[0],1),Nan_number,dtype="int64")
        index=np.arange(base_eq_id_list.shape[0])[base_eq_id_list[:,1]==n_op1]
        for i in index:
            cache[base_eq_id_list[i,2],base_eq_id_list[i,3],dict_num_to_index[base_eq_id_list[i,4]],base_eq_id_list[i,0]+4]=i 
        return_cache[n_op1]=cache
        return_dict_num_to_index[n_op1]=dict_num_to_index
        return_np_index_to_num[n_op1]=unique_num
    np.savez(f'cache_folder/cache_{max_ops}',**{str(n_op1):return_cache[n_op1] for n_op1 in range(max_ops)})
    np.savez(f'cache_folder/num_to_index_{max_ops}',**{str(n_op1):return_np_index_to_num[n_op1] for n_op1 in range(max_ops)})
...
def make_unique_equations(max_op,num_threads,random_x,before_similar_num_list,bar_format,is_print,ProgressBar_disable):
    ...
    if max_op!=7:
        ...
        for n_op1 in range(max_op-1,-1,-1):
            n_op2=max_op-1-n_op1
            print(f"   n_op1={n_op1},n_op2={n_op2}",flush=True)
            how_loop,loop_per_threads=loop_count(max_op,n_op1,num_threads,is_print=is_print)
            print(f"      make_unique_equations_thread",flush=True)
            mem_size_per_1data=(2*random_x.shape[1]*8+8+1+1)
            mem_size=((mem_size_per_1data*loop_per_threads*num_threads)//100000)/10
            print(f"         Memory size of numpy array = {mem_size} M bytes +alpha (1data={mem_size_per_1data} bytes, loop={how_loop})",flush=True)
            time1,time2=time2,datetime.datetime.now()
            with ProgressBar(total=how_loop,dynamic_ncols=False,disable=ProgressBar_disable,bar_format=bar_format,leave=False) as progress:
                TF_list,similar_num_list,need_calc_list=make_unique_equations_thread(max_op,n_op1,num_threads,random_x,
                                                                                     before_similar_num_list,saved_similar_num_list,progress)
            time1,time2=time2,datetime.datetime.now()
            print(f"         time : {time2-time1}",flush=True)
            print(f"      dim_reduction",flush=True)
            ...
            loop=np.sum(TF_list)
            print(f"         Memory size of numpy array = {mem_size} M bytes +alpha (1data={mem_size_per_1data} bytes, loop={loop})",flush=True)
            time1,time2=time2,datetime.datetime.now()
            with ProgressBar(total=how_loop,dynamic_ncols=False,disable=ProgressBar_disable,bar_format=bar_format,leave=False) as progress:
                TF_list,need_calc_list=dim_reduction(TF_list,similar_num_list,need_calc_list,progress)
            time1,time2=time2,datetime.datetime.now()
            print(f"         time : {time2-time1}",flush=True)
            print(f"      make_unique_equations_info",flush=True)
            ...
            time1,time2=time2,datetime.datetime.now()
            with ProgressBar(total=how_loop,dynamic_ncols=False,disable=ProgressBar_disable,bar_format=bar_format,leave=False) as progress:
                equation_list,similar_num_list,need_calc_list,base_eq_id_list,need_calc_change_x=make_unique_equations_info(max_op,n_op1,
                                                                                                                            TF_list,need_calc_list,random_x,progress)
            time1,time2=time2,datetime.datetime.now()
            print(f"         time : {time2-time1}",flush=True)
        ...
        make_final_cache(max_op,num_threads,saved_equation_list,saved_need_calc_list,saved_base_eq_id_list,saved_need_calc_change_x)
    else:# max_op==7
        ...
        for n_op1 in range(max_op-1,-1,-1):
            ...
            how_loop,loop_per_threads=loop_count(max_op,n_op1,num_threads,is_print=is_print)
            print(f"      make_unique_equations_thread",flush=True)
            if os.path.isfile(f"cache_folder/saved_7_make_unique_equations_thread_{n_op1}_{num_threads}.npz"):
                TF_list=np.load(f"cache_folder/saved_7_make_unique_equations_thread_{n_op1}_{num_threads}.npz")["TF_list"]
                how_loop=np.sum(TF_list)
                print(f"         make_similar_num_by_saved_7",flush=True)
                with ProgressBar(total=how_loop,dynamic_ncols=False,disable=ProgressBar_disable,bar_format=bar_format,leave=False) as progress:
                ...
                time1,time2=time2,datetime.datetime.now()
                with ProgressBar(total=how_loop,dynamic_ncols=False,disable=ProgressBar_disable,bar_format=bar_format,leave=False) as progress:
                    TF_list,similar_num_list=make_unique_equations_thread_7(n_op1,num_threads,random_x,before_similar_num_list_7,saved_similar_num_list,progress)
                time1,time2=time2,datetime.datetime.now()
                np.savez(f'cache_folder/saved_7_make_unique_equations_thread_{n_op1}_{num_threads}',TF_list=TF_list)
                print(f"         time : {time2-time1}",flush=True)
            print(f"      dim_reduction",flush=True)
            if os.path.isfile(f"cache_folder/saved_7_dim_reduction_{n_op1}_{num_threads}.npz"):
                TF_list=np.load(f"cache_folder/saved_7_dim_reduction_{n_op1}_{num_threads}.npz")["TF_list"]
                ...
                with ProgressBar(total=how_loop,dynamic_ncols=False,disable=ProgressBar_disable,bar_format=bar_format,leave=False) as progress:
                    ...
                    need_calc_list=TF_list
                time1,time2=time2,datetime.datetime.now()
                np.savez(f'cache_folder/saved_7_dim_reduction_{n_op1}_{num_threads}',TF_list=TF_list)
                print(f"         time : {time2-time1}",flush=True)
            print(f"      make_unique_equations_info",flush=True)
            how_loop=np.sum(TF_list)
            ...
            time1,time2=time2,datetime.datetime.now()
            with ProgressBar(total=how_loop,dynamic_ncols=False,disable=ProgressBar_disable,bar_format=bar_format,leave=False) as progress:
                equation_list,similar_num_list,base_eq_id_list,need_calc_change_x=make_unique_equations_info_7(n_op1,TF_list,random_x,progress)
            time1,time2=time2,datetime.datetime.now()
            print(f"         time : {time2-time1}",flush=True)
            saved_equation_list=np.concatenate((saved_equation_list,equation_list))
        ...
        make_final_cache(max_op,num_threads,saved_equation_list,saved_need_calc_list,saved_base_eq_id_list,saved_need_calc_change_x)
    return saved_equation_list,saved_need_calc_list,saved_base_eq_id_list,saved_similar_num_list

def main(max_op,num_threads,len_x=5,ProgressBar_disable=False):
    Nan_number=-100
    set_num_threads(int(num_threads))
    get_num_threads()
    time0=datetime.datetime.now()
    print(f"max_op = {max_op}  ,  date : {time0}",flush=True)
    bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]"
    #bar_format="{bar}"s
    print("use cores : ",get_num_threads(),flush=True)
    #np.random.seed(100)
    print("making random_x",flush=True)
    ...
    random_x[0]=1
    print(f"random_x corrcoef = {save_corrcoef}",flush=True)
    base_dict=cache_load(max_op-1)
    before_equations=np.array([[1],[0]],dtype="int8")
    for i in range(1,max_op):
        equations=base_dict[i]
        base=np.full((before_equations.shape[0]+equations.shape[0],equations.shape[1]),Nan_number,dtype="int8")
    ...
    print("make_before_similar_num_list",flush=True)
    dict_mask_x=make_dict_mask_x(max_op)
    loop_per_threads=(before_equations.shape[0]//num_threads+1)
    ...
    print(f"   Memory size of numpy array = {mem_size} M bytes +alpha (1data={mem_size_per_1data} bytes, loop={loop})",flush=True)
    time1,time2=datetime.datetime.now(),datetime.datetime.now()
    with ProgressBar(total=loop,dynamic_ncols=False,disable=ProgressBar_disable,bar_format=bar_format,leave=False) as progress:
        before_similar_num_list=make_before_similar_num_list(num_threads,max_op,random_x,before_equations,progress)
    time1,time2=time2,datetime.datetime.now()
    dTime=time2-time1
    print(f"   time : {dTime}",flush=True)
    print(f"make_unique_equations",flush=True)
    time1,time2=time2,datetime.datetime.now()
    equations,need_calc,base_eq_id,similar_num=make_unique_equations(max_op,num_threads,random_x,before_similar_num_list,bar_format,True,ProgressBar_disable)
    time1,time2=time2,datetime.datetime.now()
    dTime=time2-time1
    print(f"   time : {dTime}",flush=True)
    ...
    print(f"need calc  {Tcount}:{Fcount}",flush=True)
    dTime=time2-time0
    print(f"total time : {dTime}",flush=True)
